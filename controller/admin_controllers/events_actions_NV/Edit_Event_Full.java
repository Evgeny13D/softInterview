...
import A402.dao.daoImpl.EventDaoImpl;
...
/**
 * Created by Kapliarchuk_Y on 09/08/2018.
 */
@WebServlet("/Edit_Event_Full")
@MultipartConfig
public class Edit_Event_Full extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (priceString.equals("0.0") && priceS16.equals("0.0") ) {
        ...
        } else if(priceString.equals("0.0") && !(priceS16.equals("0.0"))){
            event_nv.setPrice(0);
            event_nv.setPriceS16(Double.valueOf(priceS16));
            event_nv.setPriceS712(Double.valueOf(request.getParameter("priceS712")));
            event_nv.setPriceS13(Double.valueOf(request.getParameter("priceS13")));
...
//        request.getRequestDispatcher("WEB-INF/pages/indexOfAdmin1.jsp").forward(request, response);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Event_NV event_nv = new EventDaoImpl_NV().getEventByIdJustForRemove(Integer.valueOf(request.getParameter("EventId")));
        if(event_nv.getType().equals("private")){
            request.setAttribute("event", event_nv);
            request.getRequestDispatcher("WEB-INF/pages/PrivateEvent_Edit_Ed.jsp").forward(request, response);
        } else{
            request.setAttribute("event", event_nv);
            request.getRequestDispatcher("WEB-INF/pages/Event_Edit_NV_Ed.jsp").forward(request, response);
        }
    }
}
